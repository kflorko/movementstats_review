...
We can see that the availability sample is generated within the minimum convex polygon of the used samples. *MAM: if you are using the fish as a rater in all analyses, I would move that up in the set-up section.*
```{r}
...
## HMM
Finally, we grab and plot the stationary state probabilities. This is easily done using `plotStationary()` from `momentuHMM`. *MAM: citation*
```{r, fig.show='hide'}
