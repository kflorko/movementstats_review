...
k1 <- redistribution_kernel(m1, map = fish_raster, start = start, stochastic = TRUE, tolerance.outside = 0.2,
...
n <- 30
p1 <- replicate(n, amt::simulate_path(x = k1, n = 20, start = start, verbose = TRUE), simplify = FALSE) # Takes about 2 min

uds <- lapply(c(5, 20), function(i) {
  tibble(
    rep = 1:n, 
...
ssf2pred <- hr_ud(uds[[2]]) %>% terra::as.data.frame(xy = TRUE)
