...
# combine data frame
preds2k <- data.frame(preydiv = pred$preydiv,
                     model = "rsf",
                     fit = predsk$fit, 
                     se = predsk$fit - predsk$upr)

# plot 
ggplot(data=preds2k, mapping=aes(x=preydiv,y=fit)) + geom_line() +  
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se, x=preydiv), alpha = 0.3) +
...
for(speed.i in unique(speeds)){
  ...
  predsk <- predict(ssf2_m, 
          newdata = data.frame(
            preydiv = pred$preydiv,
            log_sl = log(Par0_hmm1$Par$step[speed.i]), # SLOW SL
            cos_ta = cos(mean(na.omit(data_hmm$angle))), # mean TA
            step_id_ = 1), 
          se.fit = TRUE)
 ...
 # calculate se
  ...
  # combine data frame
  preds2_SL_slow <- data.frame(preydiv = pred$preydiv,
                     model = "rsf_SL_slow",
                     fit = predsk$fit, 
                     se = predsk$fit - predsk$upr)

# determine the step lengths with different behaviours (from hmm) to set "slow", "moderate", and "fast" speeds/step lengths for prediction
pred <- pred %>%
  mutate(log_sl_slow = log(Par0_hmm1$Par$step[1]),
         log_sl_mod = log(Par0_hmm1$Par$step[2]),
         log_sl_fast = log(Par0_hmm1$Par$step[3]),
         log_sl_mean = log(mean(na.omit(data_hmm$step))))

##-- 1. slow movement
predsk <- predict(ssf2_m, 
          newdata = data.frame(
            preydiv = pred$preydiv,
            log_sl = log(Par0_hmm1$Par$step[1]), # SLOW SL
            cos_ta = cos(mean(na.omit(data_hmm$angle))), # mean TA
            step_id_ = 1), 
          se.fit = TRUE)
