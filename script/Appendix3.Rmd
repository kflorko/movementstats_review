...
# Line plots of prey diversity vs probability of selection
...
## RSF
Since the RSF does not incorporate movement, we will calculate the log-RSS of the movement-free habitat selection kernel. 
```{r}
...
ssf_line_1 <- ggplot() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_line(data = res, aes(x = preydiv_x1, y = log_rss, color = Speed, group = Speed), size = 1, linetype = 3) + 
  geom_ribbon(data = res, aes(ymin=lwr, ymax=upr, x=preydiv_x1, fill = Speed, group = Speed), alpha = 0.3) +
  xlab("Prey diversity") +
  ylab("log-RSS") +
  theme_minimal()
ssf_line_1
...
results_ssf2 <- dplyr::bind_rows(results_ssf2) %>%
  mutate(log_sl_x1 = as.factor(round(log_sl_x1,1)),
         Speed = dplyr::case_when(as.factor(log_sl_x1) == '4421.9' ~ "slow",
                                  as.factor(log_sl_x1) == "9802.9" ~ "moderate",
                                  as.factor(log_sl_x1) == "15000.7" ~ "fast"))

# plot
ssf_line_2 <- ggplot(results_ssf2, aes(x = preydiv_x1, y = (log_rss))) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
...
<div style="text-indent: -40px; padding-left: 40px;">
McClintock, B.T., Michelot, T. 2018. momentuHMM: R package for generalized hidden markov models of animal movement. Methods Ecol. Evolut. 9, 1518-1530.  https://doi.org/10.1111/2041-210X.12995 </div>
