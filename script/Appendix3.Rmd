---
title: "An introduction to statistical models used to characterize species-habitat associations with movement data: Appendix 3"
...
A possible reason for the significant positive estimate for prey diversity may be due to autocorrlation. We will check for autocorrelation, subsample, check for autocorrelation again, and refit the model to see how the estimate changes.
...
# subsample of the data, every 10th row
data_rsf %>%
  filter(case_ == TRUE) %>%
  select(preydiv) %>%
...
We can see that the data has autocorrelation, and that after sampling every 10th row, the autocorrelation seems to be reduced. We will fit a RSF with the thinned dataset that samples every 10th row, and call it rsf2.

```{r}
data_rsf_subsampled <- data_rsf %>%
  filter(dplyr::row_number() %% 10 == 1) 

# fit rsf (a binomial logistic regression)
rsf2 <- data_rsf_subsampled %>%
  amt::fit_rsf(case_ ~ preydiv, model = TRUE)
...
We can see that after subsampling the data, prey diversity is no longer a significant covariate.
...
newfish <- fish_raster %>%
  ...
  filter(x >= 143329.9 & x <= 543329.9 & y >= -491109 & y <= -41109)
...
Now we will apply `log_rss()` to each row for rsf1. Since `log_rss()` only assessed one location relative to a reference point, we will use lapply to iterate through all locations.
...
We will now apply `log_rss()` for rsf2.

```{r rsf2 log-rss}

log_rss_list_thinned <- lapply(1:nrow(x1), function(i) {
  # Calculate log-RSS for that row
  xx <- log_rss(rsf2, x1[i,], x2[i,], ci = "se")
  # Return the element $df
  return(xx$df)
})

# combine rows
res1_thinned <- dplyr::bind_rows(log_rss_list_thinned)

# combine with rsf1
res_both <- dplyr::bind_rows(
  res1 %>% mutate(model = "rsf1"),
  res1_thinned %>% mutate(model = "rsf2"))

# plot
line_rsf2 <- ggplot(res_both, aes(x = preydiv_x1, y = log_rss, color = model, fill = model)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  ...
  ylab("log-RSS") +
  theme_minimal() +
  scale_color_manual(values = c("rsf1" = "#F8D59F", "rsf2" = "#AFC6D0")) +
  scale_fill_manual(values = c("rsf1" = "#F8D59F", "rsf2" = "#AFC6D0"))

line_rsf2

```

We can see how the predicted relationship with prey diversity has smaller error bars is smaller in the model that used all of the data (rsf1), and much larger error bars in the model that used a thinned dataset (rsf2).
...
log_rss_list_ssf1 <- lapply(1:nrow(x1), function(i) {
  # Calculate log-RSS for that row
...
# combine rows
res2 <- dplyr::bind_rows(log_rss_list_ssf1) %>%
...
line_ssf1 <- ggplot() +
  ...
  geom_line(data = res2, aes(x = preydiv_x1, y = log_rss, color = Speed, group = Speed), linewidth = 1, linetype = 3, color = "black") + 
...
We will make a combined plot with the two RSFs and this SSF.

```{r rsf1 rsf2 ssf1 log-rss plot}
# combine with rsf1
res_all <- dplyr::bind_rows(
  res1 %>% mutate(model = "rsf1"),
  res1_thinned %>% mutate(model = "rsf2"),
  res2 %>% mutate(model = "ssf1"))


# Reorder factor levels for the model column to ensure rsf models are drawn last (on top)
res_all$model <- factor(res_all$model, levels = c("ssf1", "rsf2", "rsf1"))

# plot
line_ssf1_rsfboth <- ggplot(res_all, aes(x = preydiv_x1, y = log_rss, fill = model)) +
  geom_hline(aes(group = model), yintercept = 0, linetype = "dashed", color = "gray") +
  geom_ribbon(aes(ymin = lwr, ymax = upr, group = model), alpha = 0.4) +  
  geom_line(aes(color = model), linewidth = 0.5, alpha = 0.3) +  
  xlab("Prey diversity") +
  ylab("log-RSS") +
  theme_minimal() +
  scale_fill_manual(values = c("rsf1" = "black", "rsf2" = "#FF6E6E", "ssf1" = "#3AC69D")) +
  scale_color_manual(values = c("rsf1" = "black", "rsf2" = "#FF6E6E", "ssf1" = "#3AC69D")) +
...
We can predict the estimated probability of use from the RSF with all the data by hand. First, grab the model coefficients and predict for each cell.

```{r rsf prediction}

# grab model coefficients
modcoef <- summary(rsf2)$coef
...
Visualize results for the rsf of all the data.
...
Now do the same for the rsf with the thinned dataset.

```{r rsf2 prediction}

# grab model coefficients
modcoef2 <- summary(rsf2)$coef

# prediction for each cell
x2 <- exp(modcoef2[2] * newfish$preydiv)

# set the range from zero to one
newfish$rsf_prediction2 <- range01(x2)

map_rsf2 <- ggplot() + 
  geom_tile(data = newfish, aes(x = x,y = y, fill = rsf_prediction2)) +
  scale_fill_viridis(option = "mako", name = "RSF prediction",limits = c(0,1)) +
  geom_sf(data = nat_trans, fill = "grey80", color = "white") + 
  coord_sf(xlim = ext(fish_raster)[1:2], ylim = ext(fish_raster)[3:4], expand = F) +
  theme_void()
...
We see that the two RSFs show a similar prediction map due to both having a positive coefficient estimate. However, it is important to note that the std error in the rsf of the thinned dataset is much larger.
...
We will use an expanded prey diversity raster to avoid the reflective boundary in our prediction map of the study area that may form as an artefact of the simulation.


```{r ssf1 set redistribution kernel}

# read extended fish data
fish_extended <- read.csv(here("data/fish_extended.csv"))

# rasterize prey diversity data
fish_extended_raster <- terra::rast(fish_extended, crs = "+proj=laea +lat_0=60 +lon_0=-85 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0")

k1 <- redistribution_kernel(m1, map = fish_extended_raster, start = start,
                            stochastic = TRUE,
                            tolerance.outside = 1,
                            n.control = n_steps)
...
uds_ssf1 <- tibble(rep = 1:nrow(p1_burnt), 
  ...
  filter(!is.na(x_)) %>%
  make_track(x_, y_) %>% 
  hr_kde(trast = fish_extended_raster, which_min = "global") %>%
  hr_ud() %>% 
  terra::as.data.frame(xy = TRUE)
...
The map shows that probability of use is fairly generally distributed in space, with some slightly higher predictions in the southwest and northeast regions of the study area. Note that the prediction values are small.
...
set.seed(2024)
k2 <- redistribution_kernel(m2, map = fish_extended_raster, start = start,
                            stochastic = TRUE,
                            tolerance.outside = 1,
                            n.control = n_steps)
...
The map shows that probability of use is fairly generally distributed in space, with some  higher predictions in the southeast regions of the study area. Note that the prediction values are small.
