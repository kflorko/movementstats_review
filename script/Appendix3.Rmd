...
fish_raster <- rast(fish, crs = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
...
#2. SSF
This is not easily implemented. Options exist for estimating utilization distribution from a SSF, as detailed in Signer et al. 2017. Previous versions of amt had functions for this (habitat_kernel, movement_kernel, and simulate_ud, Signer et al. 2019), but these have since been deprecated.
```{r}

ssf1 <- data_ssf %>%
  fit_clogit(case_ ~ preydiv + sl_ + cos_ta + strata(step_id_), model = TRUE)
fish_raster

# 1. make_start()
ms <- make_start(
  x = c(pull(data_ssf[1, "x1_"]), pull(data_ssf[1, "y1_"])),
  cos_ta = 0,
  log_sl = 1,
  time = pull(data_ssf[1, "t1_"]),
  dt = hours(1),
  crs = crs(fish_raster))
