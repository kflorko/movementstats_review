...
author: "Katie Florko (#MAM: add co-authors, at least those that help with the code)"
date: "22/09/2023"
output:
  html_document:
    toc: yes
...
#MAM: I think you also need rnaturalearthdata (or at least it needs to be installed also, if not you run into errors on line 73)
library(rgdal) 
#MAM: do you need rgdal. It's deprecated (which is not great). If you really need to stick to rgdal (than add how to install from archive:
...
# I think you need geosphere to be installed for l344 to work. Worth mentionning this (or just add library(geosphere), then people will know to install)
...
#MAM: I think you have multiple HMMs, no? So more than 4 models. Maybe clarify/correct previous paragraph.
...
#MAM: related to above, maybe you need to say HMMs rather than HMM.
...
```{r ssf data}
# prep data and generate availability sample
#MAM: This was great, but made a change to help explain the parameters
data_obs_steps <- seal %>%
  make_track(lon, lat, date) %>% #  convert data to track format
  steps()
set.seed(2023)
...
#MAM: I get Warning NaNs. Is this normal? If yes, can you state/explain?

#MAM: I've also added this (it's useful to understand the parameters below):
...
#MAM: Any reason why you specific the sd for retry fit rather than default values?
...
#MAM: you probabily want to add retryFits everywhere.
...
We can see from the step-length histogram and the map that it doesn't look like this HMM is capturing the long step length movements as their own state. We can look at the pseudo-residuals of the model to confirm whether this is the case. 

#MAM: to me it looks like it is capturing the long step as their own state. Also, the 3-states actually has the same long step length distribution, it's more dividing the short step length into short and moderate, no? Am I missing something? 
...
#MAM: I would not over emphasize the QQ plot from the step-length. They are actually not so bad. The turning anlge is more problematic. Your point on the autocorrelation of step length is good.
...
#### 2-states with covariate effects on observation probabilities

First we define the functions for covariates on observation probabilities of step length mean and turn angle concentration.

```{r define parameters for hmm_obs_2}
distFormula <- ~ preydiv
...
Next we will define initial parameters for this model. Note, when including parameters on observation probabilities, initial parameters must be on the working scale (i.e., from negative to positive infinity). The following parameters were obtained from refitting the model with different starting parameters and using retryFits argument in fitHMM.
...
```{r define initila pars for hmm_obs_2 observation probability}
Par0_hmm_obs_2 <- list(step = c(7, -9, 
                               8, -8,
                               5, -6, 
                               0.1, 3),
                      angle = c(7, -14,
...
```{r fit hmm_obs_2, message = FALSE}
set.seed(2023)
hmm_obs_2 <- momentuHMM::fitHMM(data=data_hmm, nbStates=2,
...
```{r plot hmm_obs_2, message = FALSE}
plot(hmm_obs_2, ask = F, plotCI = T)
plotPR(hmm_obs_2)
```

We see that when including covariates affecting the observation distributions in a 2-state model, step length of both states decreases as preydiv increases. The mean step length of slow state varies from 12 to 4 with preydiv, while for the fast state, mean step length varies from 42 to 12. For both states, the turn angle concentration also decreases with prey div. In other words, as prey diversity increases, behaviors become slower and more tortuous. The decoded state map also looks like it's adequately distinguishing faster travelling and slower resident states. Very importantly, we see that the Q-Q plots are significantly improved, with most of the data falling within the quantile bands. However, the ACF plots are not noticeably better than previous models, suggesting there is still latent autocorrelation we are not capturing. 

#MAM: I think maybe my results are slightly different. I don't see the same things in the QQ-plots. I'd say the ACF of the step length is much better (not perfect, but some of the correlation has dropped).
...
#MAM: here again, I'm not sure I would over state the qq-plots results. But definitely the ACF is fixed now!
...
#### 3-states with covariate effects on transition and observation probabilities

#MAM: you can cut this one, and add some of that info in the next one, since it's the main alternative presented in the appendix (I think).

Here we allow prey diversity to influence the step length and turning angle in the observation probabilities. The covariates allowed to influence the observation probabilities are typically abiotic (e.g., snow depth), but for the sake of this tutorial, we will fit this model using prey diversity. Note that our downstream analyses focus on the main HMM, where we allow prey diversity to influence the state transition probability.
...
```{r define parameters for hmm observation probability 3 state}
...
```{r grab parameters for hmm observation probability 3 state}

Par0_hmm_obstrans_3 <- momentuHMM::getPar0(hmm_trans_3, DM = DM)
...
```{r preview Par0_hmm_obstrans_3}
# transition probability matrix (TPM) parameters from hmm1_km 
# 'natural scale'
hmm_trans_3$mle$step
# initial parameters for hmm2_km with effect of preydiv on TPM
# working scale converted to natural scale using inverse of link function
exp(Par0_hmm_obstrans_3$Par$step)
```

Fit the model.

```{r fit hmm_obstrans_3}

# fit HMM with parameters from hmm2_km and initial slope parameters set to 0
set.seed(2023)
hmm_obstrans_3 <- momentuHMM::fitHMM(data=data_hmm, nbStates=3,
            stateNames = c("Slow movement", "Moderate movement", "Fast movement"),
            dist=list(step=stepDist,angle=angleDist),
            Par0=Par0_hmm_obstrans_3$Par,
            delta0 = Par0_hmm_obstrans_3$delta, 
            beta0 = Par0_hmm_obstrans_3$beta,
            formula=formula,
            DM = DM)
...
Get initial parameter estimates based on hmm_trans_3
...
plot(hmm_obs_3, ask = F, plotCI = TRUE)
