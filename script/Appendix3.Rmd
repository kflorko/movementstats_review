...
# habitat_kernel function

data_ssf$case_ <- as.integer(as.logical(data_ssf$case_))
ssffit <- data_ssf %>% 
  fit_issf(case_ ~ preydiv +log_sl + cos_ta + strata(step_id_))

sl_char <- amt::sl_distr(filter(data_ssf, case_ == 1))
