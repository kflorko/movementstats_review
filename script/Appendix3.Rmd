...
fish <- read.csv("preydiv_2012.csv")
head(fish)

preydiv <- fish %>%
  mutate(lon = x, lat = y) %>%
  dplyr::select(lon,lat,preydiv) %>%
  filter(lon >150000 & lon < 550000) %>%
  filter(lat > -500000 & lat < -50000) %>%
  mutate(preydiv = round(preydiv, 3))
head(preydiv)

write.csv(preydiv, "preydiv.csv")
...
## long data frame
fishl <- fish %>%
  tidyr::pivot_longer(cols = c(capelin, arccod, nsandlance), names_to = "species", values_to = "biomass")
