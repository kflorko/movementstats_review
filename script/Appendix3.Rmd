...
start <- make_start((seal %>%
                       mutate(date = as.POSIXct(date)) %>%
                       make_track(lon, lat, date))[4, ]) # This is the starting location (we could sample it randomly from within the area we want to )
...
n <- 2
p1 <- replicate(n, amt::simulate_path(x = k1, n = 240, 
                                      start = start, verbose = TRUE), 
                simplify = FALSE) # Takes about 2 min

uds <- lapply(c(5, 10), function(i) {
  tibble(
    rep = 1:n, 
...
ssf2pred <- hr_ud(uds[[1]]) %>% terra::as.data.frame(xy = TRUE)

map_SSF2 <- ggplot() + 
  geom_tile(data = ssf2pred, aes(x = x,y = y, fill = preydiv)) +
...
### ATTEMPT JULY 7 PM: SSUD
k1 <- redistribution_kernel(m1, map = fish_raster, start = start, 
                            stochastic = TRUE, tolerance.outside = 0.8,
                            n.control = 1e5, n.sample = 1000)
...
uds <- tibble(rep = 1:n_steps1, 
  ...
  make_track(x_, y_) |> 
  hr_kde(trast = fish_raster, which_min = "global")
#terra::plot(hr_ud(uds))

ssf2pred <- hr_ud(uds) %>% terra::as.data.frame(xy = TRUE)
map_SSF2 <- ggplot() + 
  geom_tile(data = ssf2pred, aes(x = x,y = y, fill = preydiv)) +
  scale_fill_viridis(option = "mako", name = "SSF2 prediction") +
  geom_polygon(data = nat_trans, aes(x=long,y=lat,group=group), fill = "grey80", color = "white") +
...
ggplot() + 
  geom_point(data = p1, aes(x = x_,y = y_)) 
