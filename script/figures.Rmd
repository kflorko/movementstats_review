...
# add proper utm
data_hmm_seal<- data_hmm %>%
  mutate(lat = seal$lat,
         lon = seal$lon)

fig4b <- p1 +
  scale_fill_viridis(option = "mako", limits = c(0.5, 0.75), name = "Prey\ndiversity") +
  geom_path(data=data_hmm_seal, aes(x=lon, y=lat, color = state, group =ID)) + 
  geom_point(data=data_hmm_seal, aes(x=lon, y=lat, color = state, shape = state), size=2, alpha = 0.8) + 
...
# Fig. 5
```{r}
res_both <- dplyr::bind_rows(res1 %>% mutate(Model = "RSF of all data"),
                             res1_thinned %>% mutate(Model = "RSF of thinned data"),
                      res2 %>% mutate(Model = "SSF")) %>%
  mutate(Model = as.factor(Model))

# Reorder factor levels for the model column to ensure rsf models are drawn last (on top)
res_both$Model <- factor(res_both$Model, levels = c("SSF", "RSF of thinned data", "RSF of all data"))

fig5a <- ggplot() + theme_minimal() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_line(data = res_both, aes(x = preydiv_x1, y = log_rss, color = Model), size = 0.5) +
  geom_ribbon(data = res_both, aes(y = log_rss, ymin=lwr, ymax=upr, x=preydiv_x1, fill = Model), alpha = 0.4) +
  scale_color_manual(values = c("#FFBF00", "#3CDBC0", "black")) +
  scale_fill_manual(values = c("#FFBF00", "#3CDBC0", "black")) +
  xlab("Prey diversity") +
  ylab("\nlog-RSS") +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size = 1) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size = 1) +
  ggtitle("A)") +
  theme(legend.position = c(0.39, 0.86), text = element_text(size=14)) +
  xlim(0.55, 0.70) +
  ylim(-2.5, 2.5) +
...
fig5b <- ggplot(results_ssf2, aes(x = preydiv_x1, y = (log_rss))) +
  ...
  geom_line(size = 0.5, aes(color = Speed, group = Speed)) +
...
fig6a <- p1 +
  ...
  scale_fill_viridis(option = "mako", name = "Prediction", limits = c(0, 1), breaks = c(0.0, 0.5, 1)) +
  geom_sf(data = nat_trans, fill = "grey80", color = "white") + 
  coord_sf(xlim = ext(fish_raster)[1:2], ylim = ext(fish_raster)[3:4], expand = F) +
  theme(legend.key.height=unit(0.3,"cm"),
        legend.key.width = unit(0.4, "cm"),
...
fig6b <- p1 + 
  ...
  scale_fill_viridis(option = "mako", name = "Prediction", 
  ...
  theme(legend.key.height=unit(0.3,"cm"),
        ...
        legend.direction = "horizontal",
        legend.position = c(0.80, 0.89)) +
  ggtitle("B) SSF without move-habitat int.") +
  ylab(" \n \n  ") + xlab("") +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size = 1) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size = 1) +
  guides(fill = guide_colourbar(title.position="top", 
...
fig6c <- p1 + 
  ...
  theme(legend.key.height=unit(0.3,"cm"),
        ...
        legend.direction = "horizontal",
        legend.position = c(0.80, 0.89)) +
  ggtitle("C) SSF with move-habitat int.") +
  ylab(" \n \n  ") + xlab("") +
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf, size = 1) +
  annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size = 1) +
  guides(fill = guide_colourbar(title.position="top", 
...
tiff(file = "figures/asus_fig6.tiff", units="in", width=13, height = 8.5, res=200)
