...
fig5a <- p1 +
  ...
  scale_fill_viridis(option = "mako", name = "RSF prediction", limits = c(0, 0.3), breaks = c(0.0, 0.15, 0.3)) +
...
fig5b <- p1 + 
  ...
  scale_fill_viridis(option = "mako", name = "SSF1 prediction", breaks = c(0,0.06,0.12)) +
  geom_polygon(data = nat_trans, aes(x=long,y=lat,group=group), fill = "grey80", color = "white") +
  coord_cartesian(xlim = c(150000,550000), ylim = c(-500000,-50000), expand = F) +
  theme(legend.key.height=unit(0.3,"cm"),
        legend.key.width = unit(0.4, "cm"),
